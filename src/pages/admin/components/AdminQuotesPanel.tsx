...
function StatusBadge({ status }: { status?: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wider ${s.className}`}
    >
      {s.label}
    </span>
  );
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <AdminMetric
          label="Convertidos"
          value={quotes.filter((quote) => quote.status === "CONVERTED_TO_ORDER").length}
          tone="success"
        />
        <AdminMetric
          ...
          value={formatBRL(
            quotes
              .filter((quote) => quote.status === "PENDING" || quote.status === "IN_REVIEW")
              .reduce((sum, quote) => sum + (quote.total || quote.estimatedPrice || 0), 0),
          )}
        />
      </div>
            ...
            {quotes.map((q) => {
              ...
              return (
                <tr key={q.id}>
                  ...
                  <td>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="truncate max-w-[180px] text-sm font-bold text-white/90">
                          {q.fileName}
                        </p>
                  ...
                  <td className="text-right">
                    ...
                    {q.quantity && q.quantity > 1 && q.unitPrice ? (
                      <p className="font-mono text-[10px] text-white/30">
                        {formatBRL(q.unitPrice)} / un · {q.quantity}x
                      </p>
                    ) : null}
                  </td>
                  ...
                  {/* DATA */}
                  <td className="text-center font-mono text-[11px] text-white/40">
