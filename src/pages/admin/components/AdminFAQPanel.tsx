      ...
      <>
        ...
        >
          <div className="flex justify-between items-center gap-3 bg-white/[0.02] p-6 rounded-[24px] border border-white/5">
            ...
            <Button
              onClick={onAddFAQ}
              className="rounded-2xl gap-2 h-9 px-4 whitespace-nowrap shrink-0 text-[10px]"
            >
          ...
          <div className="space-y-4">
            {faqs.map((f) => (
              <div
                key={f.id}
                className="bg-surface-card rounded-[32px] p-8 border border-white/5 group hover:border-white/10 transition-all"
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 rounded-full bg-primary" />
                    <h4 className="text-sm font-bold uppercase text-white/80 italic">
                      {f.question}
                    </h4>
                  </div>
                  <button
            ...
            {faqs.length === 0 && (
              <div className="py-20 text-center glass border-dashed border-white/5 rounded-[48px] text-subtle italic">
                Nenhum FAQ catalogado até o momento.
              </div>
            )}
          </div>
        </motion.div>
          ...
          {isAddingFAQ && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/95 backdrop-blur-3xl overflow-y-auto">
              <motion.div
                ...
                className="bg-surface border border-white/10 rounded-[48px] p-10 max-w-lg w-full relative my-auto"
              >
                <button
                ...
                <form onSubmit={onFAQSubmit} className="space-y-6">
                  ...
                  <Button
                    type="submit"
                    className="w-full h-16 rounded-[24px] uppercase font-black text-xs italic tracking-widest bg-primary hover:bg-primary-hover shadow-xl shadow-primary/20"
                  >
