...
export function AdminHeader({
  ...
  const search = (
    <div className="flex h-9 items-center gap-2 rounded-lg border border-white/[0.08] bg-white/[0.03] px-3 transition focus-within:border-blue-400/50 focus-within:ring-2 focus-within:ring-blue-500/10">
      ...
      <input
        type="search"
        placeholder="Buscar em pedidos, clientes..."
        value={searchTerm}
      ...
      <div className="flex min-h-16 items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <div className="flex min-w-0 items-center gap-3">
          <button
            ...
            className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-white/10 bg-white/[0.035] text-white/70 transition hover:bg-white/[0.07] hover:text-white lg:hidden"
          ...
          <div className="min-w-0">
            ...
            {activeTabDescription && (
              <p className="hidden truncate text-[11px] text-white/40 sm:block">
                {activeTabDescription}
              </p>
            )}
          </div>
        ...
        <div className="flex shrink-0 items-center gap-2">
          <div id="admin-calculator-dock" className="flex shrink-0 items-center" />
          <div className="hidden w-64 md:block xl:w-80">{search}</div>
          <Button
            size="sm"
            variant="outline"
            className={cn(
              "h-9 rounded-lg border-white/10 bg-white/[0.025] px-3 text-[11px] font-semibold text-white/65 hover:text-white",
