      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white/[0.02] p-4 sm:p-5 rounded-2xl border border-white/[0.06]">
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-dim uppercase font-bold tracking-widest">
            ...
            {filterCategory !== "ALL" &&
              ` em ${filterCategory === "_orphan" ? "Sem categoria" : filterCategory}`}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            className="rounded-xl gap-1.5 h-8 px-3 whitespace-nowrap shrink-0 text-[10px]"
          >
          ...
          >
            <div className="flex items-center gap-2">
              <button
                ...
                className="text-dim hover:text-white text-[10px] font-black uppercase"
              >
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                ...
                className="px-2 py-1 text-[10px] font-black uppercase text-dim hover:text-white"
              >
                Cancelar
              </button>
            </div>
          ...
          return (
            ...
            >
              ...
              <div className="relative aspect-square rounded-t-2xl overflow-hidden bg-black/40 border-b border-white/[0.06]">
                ...
                <div className="absolute top-3 right-3 flex gap-1.5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity z-10">
                  <button
                    ...
                    className="p-1.5 rounded-lg bg-black/60 backdrop-blur-sm border border-white/10 text-dim hover:text-blue-400 transition-colors"
                    title="Duplicar"
                  >
                  ...
                  <button
                    onClick={() => onEdit(p)}
                    className="p-1.5 rounded-lg bg-black/60 backdrop-blur-sm border border-white/10 text-dim hover:text-primary transition-colors"
                    title="Editar"
                  >
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg bg-black/60 backdrop-blur-sm border border-white/10 text-dim hover:text-red-400 transition-colors"
                    title="Excluir"
                  >
              ...
              <div className="p-4 flex flex-col flex-1">
                <h4 className="text-sm font-black uppercase mb-1 line-clamp-1 leading-snug">
                  {p.name}
                </h4>
                <p className="text-[11px] text-dim mb-3 line-clamp-2 leading-relaxed">
                  {p.description}
                </p>
                <p className="mb-3 text-[10px] font-black uppercase tracking-widest text-primary/70">
                  Produção: {p.productionMaterial ?? "PLA"}
                </p>

                {/* Stock controls */}
                <div className="flex items-center justify-between p-2.5 bg-white/[0.03] rounded-xl border border-white/[0.04] mb-3">
                  <div>
                    <p className="text-[10px] font-black uppercase text-dim tracking-widest">
                      Estoque
                    </p>
                  ...
                  <div className="flex gap-1">
                    <button
                      onClick={() => onUpdateStock(p.id, p.stock || 0, -1)}
                      className="w-7 h-7 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center text-dim hover:text-white transition-colors text-xs"
                    >
                      −
                    </button>
                    <button
                  ...
                  <div className="flex items-center gap-1.5">
                    <div
                      className={cn(
                        ...
                        p.active !== false ? "bg-green-500" : "bg-red-500",
                      )}
                    />
