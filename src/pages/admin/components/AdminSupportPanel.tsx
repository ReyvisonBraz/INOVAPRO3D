      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            <h3 className="text-sm font-black uppercase tracking-widest italic mb-6">
              Tickets de Entrada
            </h3>
            <div className="space-y-3 max-h-[70vh] overflow-y-auto no-scrollbar pr-2">
                ...
                >
                  ...
                  <p
                    className={cn(
                      ...
                      selectedTicket?.id === t.id ? "text-white" : "text-white/80",
                    )}
                  >
                    {t.userName || "Visitante"}
                  </p>
                  <p
                    className={cn(
                      "text-[9px] line-clamp-1 mt-1",
                      selectedTicket?.id === t.id ? "text-white/60" : "text-white/40",
                    )}
                  >
                    {t.message}
                  </p>
              ...
              {tickets.length === 0 && (
                <p className="text-xs text-subtle italic text-center py-10">
            ...
            {selectedTicket ? (
              <div className="bg-surface-card rounded-[48px] p-6 sm:p-10 border border-white/5 h-full flex flex-col min-h-[500px]">
                <div className="flex justify-between items-center mb-8 border-b border-white/5 pb-6">
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={() => onMarkResolved(selectedTicket.id)}
                    >
                      Marcar Resolvido
                    </Button>
                    <Button
                      variant="outline"
                      size="sm"
                      className="rounded-xl border-red-500/20 text-red-500"
                ...
                <div className="flex-1 space-y-6">
                  <div className="bg-white/5 p-6 rounded-[32px] border border-white/5">
                    <p className="text-[10px] text-dim uppercase font-black mb-2 italic">
                      Mensagem do Cliente:
                    </p>
                  ...
                  <div className="mt-auto space-y-4 pt-6">
                    <div className="flex justify-between items-center">
                      <label className="text-[10px] font-black uppercase tracking-widest text-dim">
                        Resposta Rápida (Protocolo OS)
                      </label>
                      <span className="text-[11px] font-bold text-dim uppercase italic">
                        Logs serão registrados automaticamente
                      </span>
                    </div>
                    <textarea
                    ...
                    />
                    <Button
                      className="w-full py-6 rounded-[24px] uppercase font-black italic tracking-widest gap-3"
                      onClick={onSendReply}
                    >
              ...
              <div className="glass rounded-[48px] border border-white/5 border-dashed h-full flex flex-col items-center justify-center text-subtle opacity-30 py-20 lg:py-0">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.3em]">
                  Selecione um ticket para processar
                </p>
              </div>
            )}
          </div>
