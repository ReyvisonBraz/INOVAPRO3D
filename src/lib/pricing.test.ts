...
describe("computePricing — invariantes de custo", () => {
  ...
  it("as shares somam ~100%", () => {
    ...
    const soma =
      r.shares.material + r.shares.energy + r.shares.machine + r.shares.labor + r.shares.failure;
...
describe("computePricing — preços e piso mínimo", () => {
  it("as interfaces partem do mesmo piso mínimo central", () => {
    ...
    expect(computePricing(detailedCalculatorInput)).toEqual(computePricing(quickCalculatorInput));
